...
def detect(save_img=False):
    ...
    deepsort = DeepSort('deepsort_torch/checkpoint/ckpt.t7',
                        max_cosine_dist=0.2, min_confidence=0.3,
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                # ~ Feed Detection into Tracking
                bbox = np.array(det[:, :4], dtype=np.int)
                confidence = np.array(det[:, 4], dtype=np.float)
                clss = np.array(det[:, 5], dtype=np.int)
                outputs = deepsort.update(bbox, confidence, clss)
